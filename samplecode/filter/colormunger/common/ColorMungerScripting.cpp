...
#define KEY_DISTANCE		'disT'
	...
	{ // Descriptor suite is available.  Open a token:
		...
		{ 
			...
			//写入填充颜色
			writeProcs->putIntegerProc(token, KEY_FILLCOLOR, (int32)(222<<16));
			//写入不透明度
			writeProcs->putIntegerProc(token, KEY_OPACITY, 222);
			//写入随机抖动像素距离
			writeProcs->putIntegerProc(token, KEY_DISTANCE, 222);
